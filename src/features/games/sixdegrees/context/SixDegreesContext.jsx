...
import { EXCLUDED_MOVIE_IDS } from "../api/excluded_movies_ids";
...
export const SixDegreesProvider = ({ children }) => {
      ...
      for (let i = 0; i < Math.min(5, shuffled.length - 1); i++) {
        ...
        try {
          ...
          const [response1, response2] = await Promise.all([
            fetch(
              `https://api.themoviedb.org/3/person/${actor1Id}?api_key=${apiKey}`,
            ),
            fetch(
              `https://api.themoviedb.org/3/person/${actor2Id}?api_key=${apiKey}`,
            ),
          ]);
          ...
          if (
            data1.known_for_department !== "Acting" ||
            data2.known_for_department !== "Acting"
          ) {
            continue;
          }
  ...
  const findSharedMovie = async (actor1Id, actor2Id) => {
    try {
      ...
      const [response1, response2] = await Promise.all([
        fetch(
          `https://api.themoviedb.org/3/person/${actor1Id}/movie_credits?api_key=${apiKey}`,
        ),
        fetch(
          `https://api.themoviedb.org/3/person/${actor2Id}/movie_credits?api_key=${apiKey}`,
        ),
      ]);
      ...
      const sharedMovies = movies1
        ...
        .filter((movie) => !EXCLUDED_MOVIE_IDS.has(movie.id))
        .sort((a, b) => (b.popularity || 0) - (a.popularity || 0));
  ...
  const handleSelectActor = async (selectedActor) => {
    ...
    try {
      ...
      setErrorMessage(
        "Error al validar la conexión. Por favor, intenta de nuevo.",
      );
    } finally {
      setIsValidating(false);
    }
  ...
  const useHint = async () => {
    ...
    try {
      if (hintsUsed === 0) {
        ...
        if (sharedMovie) {
          setErrorMessage(
            `💡 Pista: Busca actores de "${sharedMovie.title}" (${sharedMovie.year})`,
          );
        } else {
          const apiKey = getApiKey();
          const response = await fetch(
          ...
          if (movies.length > 0) {
            const popularMovie = movies.sort(
              (a, b) => (b.popularity || 0) - (a.popularity || 0),
            )[0];
            setErrorMessage(
              `💡 Pista: Busca actores que trabajaron con ${lastActor.name} en "${popularMovie.title}"`,
            );
          }
        }
      } else if (hintsUsed === 1) {
        const apiKey = getApiKey();
        const response = await fetch(
          `https://api.themoviedb.org/3/person/${actorB.id}/movie_credits?api_key=${apiKey}`,
        );
        const data = await response.json();
        const movies = (data.cast || [])
          .sort((a, b) => (b.popularity || 0) - (a.popularity || 0))
      ...
      setErrorMessage(
        "💡 Pista: Busca actores muy conocidos de Hollywood que trabajen en muchas películas",
      );
      setHintsUsed(hintsUsed + 1);
    }
  };
